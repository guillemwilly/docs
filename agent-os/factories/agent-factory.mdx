...
If `factory_input` does not validate, AgentOS returns 400 before the factory runs. If `factory_input` is omitted entirely, AgentOS validates the empty object `{}` against the schema: the request succeeds when every field has a default (like `ResearchInput` above), and 400s otherwise.
...
The trust split keeps authorization decisions visible at code review time. See [RequestContext](/agent-os/factories/request-context) for the full model and [JWT Role Factory](/examples/agent-os/factories/jwt-role-factory) for an end-to-end example.
...
See the [Factories reference](/reference/agent-os/factories) for the full exception hierarchy and the post-resolve behavior (id enforcement, db fallback, `store_events`, `/continue` re-invocation).

## Developer Resources

- [Factories overview](/agent-os/factories/overview)
- [RequestContext and TrustedContext](/agent-os/factories/request-context)
