...
The `tier` claim comes from verified middleware, so a client cannot escalate themselves to enterprise by editing a request body. See [RequestContext](/agent-os/factories/request-context) for the trust model and [JWT Role Factory](/examples/agent-os/factories/jwt-role-factory) for an end-to-end JWT example.
...
<Note>
Team members must be concrete `Agent` or `Team` instances. Build any per-member customization inside the team's factory.
</Note>
...
See the [Factories reference](/reference/agent-os/factories) for the full exception hierarchy and the post-resolve behavior (id enforcement, db fallback, `store_events`).
