...
The `tier` claim is read from verified middleware, so a client cannot escalate themselves by editing a request body. See [RequestContext](/agent-os/factories/request-context) for the trust model and [JWT Role Factory](/examples/agent-os/factories/jwt-role-factory) for an end-to-end JWT example.
