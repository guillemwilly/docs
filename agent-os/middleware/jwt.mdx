...
| Parameter | Description | Default |
|-----------|-------------|---------|
| `authorization` | Enable RBAC scope checking. If left `None` and `scope_mappings` is provided, RBAC is auto-enabled. | `None` (disabled unless `scope_mappings` set) |
| `verify_audience` | Verify `aud` claim matches AgentOS ID | `False` |
| `audience` | Expected audience(s) to validate the token's `aud` claim against. Accepts a string or a list of strings; the token matches if its audience matches any of them. | `AgentOS ID` |
| `user_isolation` | Opt in to per-user data isolation. When `True`, route handlers scope the database per request and enforce session/run ownership for non-admin callers. When `False`, JWT and RBAC still apply but ownership gates stay dormant. | `False` |
