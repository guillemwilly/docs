...
|-------|------|--------------|
| `run_id` | text | Runs |
| `session_id` | text | Runs |
| `status` | enum: `ok`, `error` | Runs |
| `user_id` | text | Runs, Sessions |
| `agent_id` | text | Runs, Sessions |
| `team_id` | text | Runs, Sessions |
...
```
status = ok AND agent_id = my-agent OR team_id = backend
```
