...
Agno gives you three primitives and a large set of capabilities you can attach to each.

Add session persistence and durability using storage providers (databases, files). Add memory, knowledge, and learning so your agents remember conversations, ground answers in your data, and improve over time. Connect agents to external tools using MCP, context providers, or 100+ built-in toolkits.

With the [agno mcp](/coding-agents#option-1-add-the-docs-as-an-mcp-server) installed, you can build any agentic workflow imaginable.
...
A primitive + capabilities + configuration becomes a component in your agent platform.

You can create components in code, through a no-code UI or via the AgentOS API. Components created in code are single-version. But components created via the API or no-code UI can be versioned, checkpointed and reverted as needed. The Components API lets you build, version, and operate components over HTTP.

Here's how you do each:

1. **In code.** Define agents, teams, and workflows using the SDK. Version controlled alongside the rest of your codebase.
