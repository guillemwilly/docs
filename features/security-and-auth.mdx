...
Securing an agent is a game of layers. Each layer stops a different class of attack, from unauthenticated requests at the edge down to cross-user data access deep in the stack.
...
When `authorization=True`, every endpoint requires a valid JWT except a small set of public routes: `/`, `/health`, `/info`, `/docs`, `/redoc`, `/openapi.json`, `/docs/oauth2-redirect`
