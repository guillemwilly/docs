...
When we set the `db` param, the AgentOS creates the tables and indexes on first boot.
...
Postgres-compatible managed services like [Neon](/database/neon) and [Supabase](/database/supabase) work with `PostgresDb` directly. Point `db_url` at the managed instance. Async variants (`AsyncPostgresDb`, `AsyncSqliteDb`, `AsyncMongoDb`, `AsyncMySQLDb`) are documented under [Database](/database/overview).
