...
1. **Add session storage**, so we can have conversations with our agent. Agno automatically manages session read, write and context injection for you.
2. **Add memory**, so our agent learns from our usage patterns.
...
Install the `os` extra and run with FastAPI:
...
Your AgentOS is now running at `http://localhost:8000`. API docs are at `http://localhost:8000/docs`.
...
The code above runs our agent as a service using the AgentOS. AgentOS is a FastAPI-based runtime that serves agents and related operations as REST APIs.
