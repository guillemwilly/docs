...
| `user_isolation` | `bool` | `False` | Opt in to per-user data isolation. When `True`, routes scope sessions, memory, and traces to the caller's `user_id` (from the JWT `sub` claim) and enforce run ownership for non-admin callers. Requires a database that records `user_id`. |
