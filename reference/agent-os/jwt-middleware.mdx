...
| `excluded_route_paths` | `Optional[List[str]]` | See below | Routes to skip JWT/RBAC checks. Replaces the defaults, not additive. |
| `admin_scope` | `Optional[str]` | `"agent_os:admin"` | Scope that grants full admin access |
| `user_isolation` | `bool` | `False` | Opt in to per-user data isolation. When `True`, route handlers scope the database per request and enforce session/run ownership for non-admin callers. When `False`, JWT and RBAC still apply but ownership gates stay dormant. |
