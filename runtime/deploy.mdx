...
| Long-running runs | Use `background=true` on the run endpoint, then poll for completion (see [Serve as an API](/runtime/serve-as-api)) |
...
- At least one [interface](/runtime/interfaces) wired up
- [Pre-hooks](/runtime/human-approval) for PII or injection guarding if you handle untrusted input
