...
Callable factories let you supply callables for members, tools, or knowledge. Agno resolves them at run time with access to the run context and uses their return values for that run. This enables per-run or per-session configuration, deferred setup, and dynamic team composition.

| Behavior | Details |
|----------|---------|
| Injected parameters | `team`, `run_context`, `session_state` when present in the factory signature |
| Caching | `cache_callables=True` caches by custom key, then `user_id`, then `session_id`, see [caching settings](./building-teams#callable-factories)|
