...
Cases live in `evals/cases.py`. Each case is a prompt, the agent that should answer it, and a criterion the answer must satisfy.

```python evals/cases.py lines
...
`criteria` is graded by an LLM judge. `expected_tool_calls` checks that the agent actually used the tools you expect.
...
The blog version of this template wires the weekly run into the platform's own scheduler. See [scheduling](/runtime/scheduling) for the cron API, and the next page for production setup.
