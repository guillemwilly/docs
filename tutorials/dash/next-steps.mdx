...
See [Scheduling](/runtime/scheduling) for the patterns. The [Coda template](/tutorials/coda/overview) is the working example to copy: it registers daily digest, issue triage, and repo sync schedules in `app/main.py`. The same pattern works for Dash.
...
| Building a fully custom AgentOS app | [AgentOS Runtime](/runtime/overview) |