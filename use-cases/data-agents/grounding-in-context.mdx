...
description: "Ground every query in validated SQL, table metadata, and business rules so the agent answers in your terms, not the model's."
---

A model knows SQL. It does not know that `revenue` was replaced by `revenue_v2` last March, that "active" excludes trialing accounts, or that MRR is computed net of credits. Grounding supplies that. The agent retrieves the relevant context for each question before it writes a line of SQL.
...
| See all six layers in one agent | [Example: Dash](/use-cases/data-agents/example-dash) |
