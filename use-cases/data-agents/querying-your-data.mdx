...
description: "Give an agent read-only SQL access, schema introspection, and the discipline to cite the query it ran."
---

`SQLTools` connects an agent to a database. Point it at a read-only connection and the agent can introspect the schema and run queries, but it cannot write.
...
agent = Agent(
    ...
    tools=[
        SQLTools(
            db_url="postgresql+psycopg://readonly@warehouse/analytics",
            read_only=True,
        )
    ],
    instructions=(
...
An answer without its SQL is unverifiable. Instruct the agent to return the query alongside the result so a human can check it and a downstream system can re-run it.

```python
instructions = """\
For every answer:
1. Introspect the relevant tables.
2. Write and run the SQL.
3. Return the number and the exact SQL in a fenced block.
Never present a figure without the query that produced it.
"""
```

## Scope the connection

The cleanest boundary is the connection itself. A `read_only=True` engine physically cannot write, regardless of what the model is prompted to do. For the full read vs write split, see [Safe data access](/use-cases/data-agents/safe-data-access).
