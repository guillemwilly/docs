...
description: "Read-only and read-write boundaries enforced by the database engine, not by a prompt."
---
...
`read_only=True` on a connection scoped to a read-only database role means a generated `DROP TABLE` fails at the engine, before it reaches your data. Defense does not depend on the model behaving.
...
| Approve sensitive actions | [Human approval](/runtime/human-approval) |
| See the dual-schema split live | [Example: Dash](/use-cases/data-agents/example-dash) |
