...
Construct a provider with `write=False` and the write tool is not created. The agent cannot mutate the source under any prompt. This is how a style guide or a reference corpus stays immutable while still being queryable.
