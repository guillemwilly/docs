...
description: "Turn the agent into an HTTP service with streaming, sessions, and auth, then call it from any surface."
---

`AgentOS` wraps the agent in a FastAPI app. Every registered agent gets run, session, and memory endpoints. Your product surfaces call those endpoints.
...
curl -X POST http://localhost:7777/agents/copilot/runs \
  ...
  -d '{"message":"Summarize this thread","user_id":"sarah@acme.com","session_id":"thread-42"}'
```
...
AgentOS validates the token before any agent code runs. The `user_id`, `session_id`, and scope claims are read from the token and injected into the run, so a surface never passes identity in the request body. RBAC scopes are enforced per endpoint.

## Next steps

| Task | Guide |
|------|-------|
| Drive product widgets off this API | [Agentic product surfaces](/use-cases/product-agents/agentic-product-surfaces) |
