...
| Value | Behavior |
|-------|----------|
| `OnReject.else_branch` | Execute `else_steps` (default for Condition) |
...
Condition(
    ...
    evaluator=lambda step_input: True,  # Ignored when requires_confirmation=True
...
To use programmatic evaluation without HITL, set `requires_confirmation=False` (default):

```python
Condition(
    name="auto_evaluated",
    evaluator=lambda step_input: "urgent" in step_input.input.lower(),
    steps=[Step(name="urgent_path", ...)],
    else_steps=[Step(name="normal_path", ...)],
)
```

## Nested Conditions

Conditions can be nested. Each pauses independently:
...
Condition(
    ...
    steps=[
        Condition(
            name="inner",
            steps=[Step(name="deep", ...)],
            else_steps=[Step(name="shallow", ...)],
            requires_confirmation=True,
            confirmation_message="Go deeper?",
        ),
    ],
    else_steps=[Step(name="skip_all", ...)],
    requires_confirmation=True,
    confirmation_message="Enter analysis?",
)
```

Handle nested requirements in sequence as the workflow progresses.
