...
| Value | Behavior |
|-------|----------|
| `OnReject.skip` | Skip the loop and continue (default) |
...
Loops can contain other loops. Each pauses independently:
...
Loop(
    ...
    steps=[
        Loop(
            ...
            max_iterations=3,
            requires_confirmation=True,
            confirmation_message="Run inner loop?",
        ),
    ],
    max_iterations=2,
    requires_confirmation=True,
    confirmation_message="Run outer loop?",
)
```

The outer loop pauses first. If confirmed, it runs. When the inner loop is reached, it pauses for its own confirmation.
