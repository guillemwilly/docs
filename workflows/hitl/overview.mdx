...
description: Pause workflow execution for user confirmation, input, review, or decisions at the step level or executor level.
keywords: [workflow, HITL, human-in-the-loop, confirmation, user input, pause, step, executor, human review]
mode: wide
---

Human-in-the-Loop (HITL) in Workflows enables you to pause execution to collect user confirmation, input, or decisions. The workflow state is persisted, allowing you to resume execution after the user responds.

Workflows support HITL at two levels:

- **Step-level HITL** — pauses *before* or *after* a workflow step executes. Configured on the workflow primitive (`Step`, `Loop`, `Router`, `Condition`, `Steps`).
- **Executor-level HITL** — pauses *during* step execution when an agent or team inside the step calls a tool that requires human approval. Configured on the tool itself (e.g. `@tool(requires_confirmation=True)`). The pause propagates from the agent/team up to the workflow.

Both levels can be combined on the same step. See [Executor HITL](/workflows/hitl/executor) and [Dual-level HITL](/workflows/hitl/dual-level) for details.
...
The two can be combined on the same step. For example: `requires_confirmation=True` on a `Step` (gate before the agent runs) + `@tool(requires_confirmation=True)` on the agent's tool (gate the tool call). See [Dual-level HITL](/workflows/hitl/dual-level).
...
<CardGroup cols={3}>
  ...
  <Card title="Dual-level HITL" icon="layer-group" href="/workflows/hitl/dual-level">
