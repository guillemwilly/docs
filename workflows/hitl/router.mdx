...
| `on_reject` | `OnReject` | Action when rejected: `skip`, `cancel` (default) |
...
Routers can contain other routers or complex steps:
...
Router(
    ...
    choices=[
        Router(
            ...
            choices=[
                Step(name="quick", executor=quick),
                Step(name="deep", executor=deep),
            ],
            requires_user_input=True,
        ),
        Step(name="skip_analysis", executor=skip),
    ],
    requires_user_input=True,
)
```

Each router pauses independently. Handle nested requirements in sequence.
