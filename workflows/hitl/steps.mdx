...
| Value | Behavior |
|-------|----------|
| `OnReject.skip` | Skip the pipeline and continue (default) |
...
## Steps with Individual HITL

Steps within a pipeline can have their own HITL configuration:

```python
Steps(
    name="processing_pipeline",
    steps=[
        Step(name="validate", executor=validate),
        Step(
            name="transform",
            executor=transform,
            requires_confirmation=True,
            confirmation_message="Transform data?",
        ),
        Step(name="save", executor=save),
    ],
    requires_confirmation=True,
    confirmation_message="Run processing pipeline?",
)
```

The pipeline pauses first. If confirmed, it starts executing. When the `transform` step is reached, it pauses again for its own confirmation.
...
Steps(
    name="outer_pipeline",
    steps=[
        Step(name="prepare", executor=prepare),
        Steps(
            name="inner_pipeline",
            steps=[
                Step(name="process_a", executor=process_a),
                Step(name="process_b", executor=process_b),
            ],
            requires_confirmation=True,
            confirmation_message="Run inner pipeline?",
        ),
        Step(name="finalize", executor=finalize),
    ],
    requires_confirmation=True,
    confirmation_message="Run outer pipeline?",
)
```

Each pipeline pauses independently when reached.
